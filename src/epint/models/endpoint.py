...
class Endpoint:
    ...
    def _extract_debug_param(self, kwargs: Dict[str, Any]) -> bool:
        """debug parametresini kwargs'tan çıkarır ve boolean değerini döndürür

        Args:
            kwargs: Endpoint parametreleri dictionary'si

        Returns:
            debug parametresinin boolean değeri (varsayılan: False)
        """
        debug = False
        debug_keys = ['debug', 'Debug', 'DEBUG']
        ...
        for key in debug_keys:
            ...
            if key_lower in kwargs_keys_lower:
                original_key = kwargs_keys_lower[key_lower]
                debug = bool(kwargs.pop(original_key))
                break

        # Direkt eşleşme yoksa fuzzy matching yap
        if not debug and kwargs:
            matched_key = find_closest_match('debug', list(kwargs.keys()), threshold=0.7)
            if matched_key:
                debug = bool(kwargs.pop(matched_key))
    ...
    def __call__(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        # debug parametresini kontrol et ve kwargs'tan çıkar
        debug = self._extract_debug_param(kwargs)
        ...
        try:
            ...
            # all_data parametresi verildiyse hem pagination hem date range için işle
            if all_data:
                result_data = self.data_fetcher.fetch_all_data(
                    kwargs,
                    request_model,
                    self.__call__,
                    first_response=result_data if isinstance(result_data, dict) else None,
                    debug=debug
                )
            ...
            if all_data:
                ...
                from ..modules.data_fetcher.error_detector import is_date_range_error
                is_date_error = is_date_range_error(e)

                if is_date_error:
                    # Date range hatası - recursive olarak tüm tarih aralıklarını ve sayfaları topla
                    result = self.data_fetcher.handle_date_range_error(
                        kwargs,
                        request_model,
                        self.__call__,
                        debug=debug
